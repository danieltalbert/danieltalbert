...
- [x] Knowledge charge v1: quiz prompt in combat charges a special ability *(built to Danny's design: the focus special is a combined Kern+Bit attack CAST by answering — Q at part-charge opens a code-built quiz card (slow-mo + safe, real-time countdown, difficulty-gated per WORLDBOOK, explanation shown every answer); correct answers feed the existing meter and the strike auto-fires when it fills; wrong/timeout fizzles but keeps focus; Bit flies in to channel with new in-voice lines. New `QuizPicker` + `KnowledgePrompt`; wired through the milestone-6 hooks (`quiz_answered`→`add_charge`).* **SEEN & VERIFIED 2026-07-29** — the UNSEEN flag is cleared. A dev probe (`src/dev/channel_probe.gd`, `-- --test-channel`) drives the real input path through all 13 scenarios — open, correct, wrong, timeout, break-off, fill-and-fire, death, empty bank, double-open, invulnerability release, modal exclusion, mastery ordering, bank health — and passes **54/54** on Godot 4.7.1. `-- --shot-channel=DIR` photographs the card in each state; four frames are in `docs/progress/milestone7_channel_*.png` and were inspected. Fixed in the pass: **the cast could not be broken off at all** (card and PlayerCombat both polled `special`, on the process and physics clocks, so every cancel re-opened the channel one tick later — the key now has one reader that forwards intent); the card collided with the pack/notebook/dialogue, and the pack's `get_tree().paused` froze the countdown while wall-clock ran it down (new `UiModality` rule); Bit's opening line was dropped by a state guard and her world label ghosted through the panel; the card jumped ~70 px on every answer. Added: world scrim + entrance on wall-clock, an in-card focus meter reading "N more answers to forge the strike", Bit's voice on the card, and mastery-weighted selection — unseen questions first, then ones Kern got wrong, persisted in `GameState.flags` with no SAVE_VERSION bump. The combined strike is its own effect now (`FocusStrike`): Bit's motes spiral in, a blade of light goes up, and two shockwave ribbons race out to exactly `SPECIAL_RADIUS` so the ring teaches the real reach — `docs/progress/milestone7_strike.png`)*
